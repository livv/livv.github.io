    ...
    applyTranslations() {
        ...
        document.querySelectorAll('[data-i18n-title-key]').forEach(element => {
            ...
            if (translation && translation !== key) {
                ...
                element.removeAttribute('data-i18n-title-key');
            }
        });

        document.querySelectorAll('[data-i18n-meta-key]').forEach(element => {
            const key = element.getAttribute('data-i18n-meta-key');
            const translation = this.translate(key);
            if (translation && translation !== key) {
                ...
                element.removeAttribute('data-i18n-meta-key');
            }
        });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (langSwitchBtn) {
        langSwitchBtn.textContent = i18nManager.translate('lang.switch');

        langSwitchBtn.addEventListener('click', function(e) {
            e.preventDefault();
            i18nManager.toggleLanguage();
            i18nManager.applyTranslations();
            this.textContent = i18nManager.translate('lang.switch');
        });
    }
});